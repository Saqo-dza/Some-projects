...
namespace OmdbApi
{
    public class Graph<T>
    {
        ...
        //for omdb
        public void AddMovieForActor(T actname, string moviename)
        {
            if (Find(actname) == null)
            {
        ...
        public bool IsConectionExist(T val1,T val2)//for omdb
        {
            if (Find(val1)== null || Find(val2)==null)
            {
                throw new NullReferenceException("Elements not found");
            }
            else if(Find(val1).Neighbors.Contains(Find(val2)) && Find(val2).Neighbors.Contains(Find(val1)))
            {
                return true;
            }
            return false;
